...
results.dir <- file.path(base.dir, "eCog_output/LiDAR_areas/LiDAR_Results/Class3_noBT", fsep = .Platform$file.sep)   ## results directory (outputs go here)
...
#remove building and trees points
Building_index_onem <- which(points.clean@data$One_m_Class_2_1st_choice == "Buildings")
points.clean <- points.clean[-Building_index_onem,]

Building_index_fivem <- which(points.clean@data$Five_m_Class_2_1st_choice == "Buildings")
points.clean <- points.clean[-Building_index_fivem,]

Trees_index_onem <- which(points.clean@data$One_m_Class_2_1st_choice == "Trees")
points.clean <- points.clean[-Trees_index_onem,]

Trees_index_fivem <- which(points.clean@data$Five_m_Class_2_1st_choice == "Trees")
points.clean <- points.clean[-Trees_index_fivem,]

summary(points.clean@data)
points <- points.clean
...
#     foreach (tile.idx = 1:length(params$tile.names), .packages=list.of.packages) %dopar% {   
      ...
      print(dim(objects_clip_short@data)) 
      #remove huge MaxHt - MinHt values
      dropindex <- which(objects_clip_short$MaxHtMinHt > 10000)
      if(length(dropindex) >= 1){
      ...
      ##test to see if moving the if statements that filter points by ground truth will work below
